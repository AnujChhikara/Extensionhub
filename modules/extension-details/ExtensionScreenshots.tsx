...
export function ExtensionScreenshots({ extensionId }: ExtensionScreenshotsProps) {
  const extension = extensionData.find(ext => ext.id === extensionId);
  const [currentIndex, setCurrentIndex] = useState(0);

  if (!extension || !extension.screenshots || extension.screenshots.length === 0) {
    return (
      <motion.div
  ...
  const prevSlide = () => {
    setCurrentIndex((prev) => (prev - 1 + extension.screenshots.length) % extension.screenshots.length);
  };
        ...
        <div className="aspect-video bg-white/5 rounded-lg overflow-hidden">
          <div className="w-full h-full bg-gradient-to-br from-gray-600 to-gray-800 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-white font-medium">Screenshot {currentIndex + 1}</p>
              <p className="text-gray-400 text-sm mt-1">Demo of {extension.name}</p>
            </div>
          </div>
        </div>
